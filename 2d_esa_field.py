""" Generates Figure 3 of the paper
    ...
    2D synthesis of a line source with the equivalent scattering approach
...
src_angles = [135]  # incidence angle of virtual source
R = 4  # distance of virtual source
xref = [2, -2, 0]  # reference point for normalization
...
def compute_sound_field(x0, n0, a0, omega, angle):
    ...
    if wfs:
        ...
        d = sfs.mono.drivingfunction.esa_edge_2d_line(omega, x0, xs, Nc=Nc)  
        a = np.ones(d.shape[0])
...
def plot_sound_field(p, xs, twin):
    ...
    if save_figures:
        if wfs:
            myfig.savefig('../paper/figs/edge_wfs_2d_line_%dHz_%d.pdf' % (f, src_angles[n]),  transparent=True)
        else:
            myfig.savefig('../paper/figs/edge_esa_2d_line_%dHz_%d.pdf' % (f, src_angles[n]),  transparent=True)
...
normalization = np.abs(sfs.mono.source.line(omega, xs, [0, 0, 0], xref))
